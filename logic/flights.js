function Flights() {
    ...
    var checkAircraftRevision = (distanceLimit, distancesArray) => {
        var totalDistance = 0
        distancesArray.forEach((d) => totalDistance += d)

        if(totalDistance > distanceLimit) throw new Error("Flight maximum allowed distance (" + distanceLimit + ") exceeded. No flight is allowed any longer, you need to make the revision immediately.")
